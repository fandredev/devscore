  ...
  return (
    <>
      ...
      <Container>
        ...
        <section>
          <Title id="question_now">
            2. <abbr title="Hyper Text Markup Language">HTML</abbr> é uma
            linguagem de programação. Essa afirmação é:
          </Title>
          {mappedMultiChoices.map(({ value }, index: number) => (
            <OptionChoice key={index}>
              {value === true ? "Verdadeira" : "Falsa"}
            </OptionChoice>
          ))}
          <_Button bg={colors.green} id="confirm_question">
            Confirmar
          </_Button>
        </section>
        <section>
          ...
          <figure>
            <Image
              src="/images/menEyeTheCode.svg"
              alt="Homem olhando uma tela de perguntas"
              width={400}
              height={400}
            />
          </figure>
